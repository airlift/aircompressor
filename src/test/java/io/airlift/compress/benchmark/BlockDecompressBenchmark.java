...
@Warmup(iterations = 5)
@Fork(3)
public class BlockDecompressBenchmark
{
    @Param({
            "airlift_lz4",
            "airlift_snappy",
    ...
    @Benchmark
    public int blockDecompress(BytesCounter counter)
    {
        int written = decompressor.decompress(compressed, 0, compressed.length, uncompressed, 0, uncompressed.length);
            ...
            throws RunnerException
    {
        Options opt = new OptionsBuilder()
                .include(".*\\." + BlockDecompressBenchmark.class.getSimpleName() + ".*")
                .build();
