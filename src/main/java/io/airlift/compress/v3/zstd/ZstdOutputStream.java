...
{
    static ZstdOutputStream create(OutputStream out)
            throws IOException
    {
    ...
    static ZstdOutputStream create(OutputStream out, int compressionLevel)
            throws IOException
    {
        if (ZstdNativeCompressor.isEnabled()) {
            return new ZstdNativeOutputStream(out, compressionLevel);
        }
