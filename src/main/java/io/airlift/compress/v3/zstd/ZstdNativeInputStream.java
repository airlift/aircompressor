 ...
 * <p>
 * This native wrapper does not add any additional buffering, other than a small buffer for reading
 * compressed data from the underlying {@link InputStream}. The decompression is performed directly
 * into the caller's output buffer. For optimal performance, callers should provide sufficiently
 * large output buffers to minimize the number of calls into native code, or should wrap this stream
 ...
 * <p>
 * This class is not thread-safe for concurrent access, but can be used from different
 * threads sequentially (e.g., in cooperative scheduling environments like Trino).
 */
        ...
        extends ZstdInputStream
{
    ...
    private boolean frameComplete = true;
        ...
        while (totalRead < outputLength) {
            ...
            if (inputPosition >= inputLimit) {
                ...
                if (bytesRead == -1) {
                    endOfStream = true;
                    if (!frameComplete) {
                        throw new IOException("Truncated zstd stream");
                    }
                    return totalRead > 0 ? totalRead : -1;
                }
                frameComplete = false;
