...
import static io.airlift.compress.v3.zstdFFM.FiniteStateEntropy.MAX_SYMBOL;
import static io.airlift.compress.v3.zstdFFM.FiniteStateEntropy.MIN_TABLE_LOG;
...
class FseTableReader
{
    ...
    public int readFseTable(FiniteStateEntropy.Table table, MemorySegment inputBase, long inputAddress, long inputLimit, int maxSymbol, int maxTableLog)
    {
        ...
        int bitStream = inputBase.get(JAVA_INT, input);
        ...
        while (remaining > 1 && symbolNumber <= maxSymbol) {
            if (previousIsZero) {
                ...
                while ((bitStream & 0xFFFF) == 0xFFFF) {
                    ...
                    if (input < inputLimit - 5) {
                        input += 2;
                        bitStream = (inputBase.get(JAVA_INT, input) >>> bitCount);
                    }
                    else {
                        // end of bit stream
                        bitStream >>>= 16;
            ...
            bitStream = inputBase.get(JAVA_INT, input) >>> (bitCount & 31);
        }
